...
type ProductAuxMaterial struct {
	...
	CgPrice               float64                             `json:"cg_price"`                // 采购价格（RMB）
...
type UpsertProductAuxMaterialRequest struct {
	...
	CgPrice            float64                                 `json:"cg_price,omitempty"`              // 采购价格（RMB）
	...
	SupplierQuote      []UpsertProductAuxMaterialSupplierQuote `json:"supplier_quote,omitempty"`        // 采购商（2021.11.20 号开始启用；不传该参数则清空产品供应商报价，否则覆盖）
...
type UpsertProductAuxMaterialSupplierQuote struct {
	ERPSupplierId      string                                      `json:"erp_supplier_id"`      // 领星 ERP 供应商 ID
	SupplierId         string                                      `json:"supplier_id"`          // 客户系统供应商 ID，没有填这个值或者对应供应商不存在，则取 erp_supplier_id
	SupplierProductURL []string                                    `json:"supplier_product_url"` // 采购链接，字符串数组，最多20个，没有则传空数组
	IsPrimary          bool                                        `json:"is_primary"`           // 首选供应商（0：否、1：是）
...
type UpsertProductAuxMaterialSupplierQuoteItem struct {
	Currency   string                                               `json:"currency"`    // 报价币种（目前只有 CNY 和 USD）
	IsTax      bool                                                 `json:"is_tax"`      // 是否含税（0：否、1：是）
	TaxRate    int                                                  `json:"tax_rate"`    // 税率（0 ~ 99 的整数，为空则表示为 0）
...
type UpsertProductAuxMaterialSupplierQuoteItemStepPrice struct {
	Moq          int     `json:"moq"`            // 最小起订量，最小值为 1
	PriceWithTax float64 `json:"price_with_tax"` // 税单价，4 位小数
}
